/* Levenshtein Distance Algorithm implementation as MySQL UDF
   ...
   Redistribute as you wish, but leave this information intact.
	...
	CASE-INSENSITIVE version created by Viliam Simko vlx@matfyz.cz 2008-08-28
*/
...
extern "C" {
	my_bool		levnocase_init(UDF_INIT *initid, UDF_ARGS *args,
	                             char *message);
	void			levnocase_deinit(UDF_INIT *initid);
	longlong		levnocase(UDF_INIT *initid, UDF_ARGS *args,
...
/******************************************************************************
** purpose:		called once for each SQL statement which invokes LEVENSHTEIN();
**					checks arguments, sets restrictions, allocates memory that
**					will be used during the main LEVENSHTEIN() function (the same
...
**					other functions (levenshtein() and levenshtein_deinit()) -
...
**					to each invocation of levenshtein(); pointer to a char
...
******************************************************************************/
my_bool levnocase_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
	int *workspace;
	...
	** value of the LEVENSHTEIN() function */
	initid->max_length = 3;

	/* levenshtein() will not be returning null */
	initid->maybe_null = 0;
...
/******************************************************************************
** purpose:		deallocate memory allocated by levenshtein_init(); this func
**					is called once for each query which invokes LEVENSHTEIN(),
**					it is called after all of the calls to levenshtein() are done
** receives:	pointer to UDF_INIT struct (the same which was used by
**					levenshtein_init() and levenshtein())
** returns:		nothing
******************************************************************************/
void levnocase_deinit(UDF_INIT *initid)
{
...
******************************************************************************/
longlong levnocase(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                     char *error)
{
	/* get a pointer to the memory allocated in levenshtein_init() */
	...
	*/
  	...
  	n = (args->args[0] == NULL) ? 0 : args->lengths[0];
  	m = (args->args[1] == NULL) ? 0 : args->lengths[1];

  	if(n != 0 && m != 0)
  	{
  	
		/* LOWER-CASE LEVENSHTEIN implemented by Viliam Simko vlx@matfyz.cz */
		/* s is the first user-supplied argument; t is the second
		** the levenshtein distance between s and t is to be computed */
	  	char s[n+m+2]; // should be deallocated automatically by the compiler
  		for(i=0;i<n;++i) s[i] = tolower(args->args[0][i]);
		s[n] = 0;
		
	  	char *t = s+n+1;
  		for(i=0;i<m;++i) t[i] = tolower(args->args[1][i]);
		t[m] = 0;
