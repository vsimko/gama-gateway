...
version:  2008-08-04 (Addition: toLegacyXML, toLegacyJSON, toLegacyHTML, toHTML methods)
*/
...
class ARC2_Class {
  ...
  function toLegacyXML($v, $ns = '') {
    ...
    $conf = array('ns' => $ns ? $ns : isset($this->a['ns']) ? $this->a['ns'] : array());
    $ser = new ARC2_LegacyXMLSerializer($conf, $this);
  ...
  function toLegacyHTML($v, $ns = '') {
    ...
    $conf = array('ns' => $ns ? $ns : isset($this->a['ns']) ? $this->a['ns'] : array());
    $ser = new ARC2_LegacyHTMLSerializer($conf, $this);
    return $ser->getSerializedArray($v);
  }
  ...
  function toHTML($v, $ns = '') {
    ...
    $conf = array('ns' => $ns ? $ns : isset($this->a['ns']) ? $this->a['ns'] : array());
    $ser = new ARC2_POSHRDFSerializer($conf, $this);
    return (isset($v[0]) && isset($v[0]['s'])) ? $ser->getSerializedTriples($v) : $ser->getSerializedIndex($v);
  }
