...
version:  2008-08-04 (Addition: Placeholders are allowed as triple blocks
                      Addition: toUTF8 call in xString)
*/
...
class ARC2_TurtleParser extends ARC2_RDFParser {
  ...
  function xPlaceholder($v) {
    if ($r = $this->x('(\?|\$)\{([^\}]+)\}', $v)) {
      ...
      if (!in_array($r[2], $this->r['placeholders'])) $this->r['placeholders'][] = $r[2];
      return array(array('value' => $r[2], 'type' => 'placeholder'), $r[3]);
    }
    return array(0, $v);
  }
